...
DETECT_IMGSZ = 544                 # 512-640 oralig'i yaxshi
USE_HALF = True                    # CUDA bo'lsa yarim aniqlik

# (ixtiyoriy) DEVICE-GATING: classifier gapi "chegarada" bo'lsa, best2.pt bilan tekshirib tasdiqlash
USE_DEVICE_GATING = False
DEVICE_MODEL_PATH = "best2.pt"
DEVICE_CONF = 0.70
GATE_ADD_THRESH = 0.10             # gating bo'lmasa thresholdni +0.10 qattiqlashtir
...
# TEZLIK
CLASSIFY_EVERY = 6      # har track uchun har 6-kadrda qayta klassifikatsiya
MOVE_IOU_TH = 0.20      # box anchagina siljisa qayta klas qil
SHOW_FPS = True
...
def build_idx_map(classes):
    ...
    def find_any(cands):
        ...
        return None
    no_dev_syn={"nodevice","no_device","none","normal","person"}
    ...
    if None in (idx_no, idx_cr, idx_wh):
        raise ValueError(f"Klass nomi mos kelmadi. Checkpoint classes={classes}")
...
def load_attr_model(weights_path, device):
    ckpt = torch.load(weights_path, map_location="cpu")  # classes ham kerak
    classes = ckpt["classes"]
    model = models.mobilenet_v3_small(weights=None)
    model.classifier[3] = nn.Linear(model.classifier[3].in_features, len(classes))
    model.load_state_id = model.load_state_dict(ckpt["model"])
    model.eval().to(device)
    # Eslatma: train’da normalization ishlatmagan bo’lsang, bu yerda ham shart emas
...
def box_center(b): x1,y1,x2,y2=b; return (0.5*(x1+x2), 0.5*(y1+y2))
def center_in_bottom_fraction(cx, cy, pbox, frac=0.6):
    x1,y1,x2,y2=pbox; y_cut = y1+(y2-y1)*(1.0-frac)
    return (x1<=cx<=x2) and (cy>=y_cut)

def gate_match(pbox, dboxes):
    # oddiy: eng yaqin device markazi person pastki 60% ichida bo'lsa True
    if not dboxes: return False
    pcx,pcy = box_center(pbox)
    for d in dboxes:
        dcx,dcy = box_center(d)
        if center_in_bottom_fraction(dcx, dcy, pbox, 0.6):
            return True
    return False
...
def main():
    ...
    while True:
        ...
        # (opt) device gating uchun boxlar
        devs = None
        if USE_DEVICE_GATING:
            db = device_boxes(frame, device_model, dev_names)
            devs = {"crutches": db.get("crutches", []), "wheelchair": db.get("wheelchair", [])}

        # 3) Qaysilarni qayta-klassifikatsiya qilish kerak?
        need = []
        for (pid, pbox, _) in tracked_persons:
            ...
            if c is None:
                ...
                # har CLASSIFY_EVERY kadrda yoki box kuchli o'zgarsa
        ...
        # 5) Natijani keshga yozish (yoki avvalgi qiymatni ishlatish)
        for (pid, pbox, _) in tracked_persons:
            if pid in results:
                ...
                # faqat boxni va frame'ni yangilaymiz
                cache[pid]["last_box"] = pbox
                cache[pid]["last_frame"] = frame_idx

        # 6) Hysteresis va chizish
        for (pid, pbox, _) in tracked_persons:
            c = cache.get(pid)
            if not c:
            ...
            # threshold (device-gating bo'lsa – qat'iyroq)
            th_cr = TH_CRUTCHES
            th_wh = TH_WHEELCHAIR
            if USE_DEVICE_GATING:
                # agar mos device topilmasa, thresholdni biroz oshiramiz
                if label == "uses_crutches" and not gate_match(pbox, devs["crutches"]):
                    th_cr += GATE_ADD_THRESH
                if label == "wheelchair_user" and not gate_match(pbox, devs["wheelchair"]):
                    th_wh += GATE_ADD_THRESH
        ...
        cv2.imshow("Person Attribute Pipeline (FAST)", frame)
