...
# ======== CONFIG ========
VIDEO = "123.mp4"   # yoki papkadan ham o'qish mumkin
OUT   = r"C:/Users/dalab/Desktop/Abbos/SmartLight/crops/no_device_3"
STEP  = 3                 # har 3-kadrda bir marta
PERSON_CONF = 0.50
DEVICE_CONF = 0.50        # pastroq: miss kamaysin
MIN_AREA    = 40*40
NMS_IOU     = 0.45
BOTTOM_FRAC = 0.70        # pastki 70% hudud
EXPAND_PBOX = 0.10        # person bbox'ni 10% kengaytirib tekshirish
VALID_DEVICES = {"crutches","wheelchair"}  # sening custom klass nomlaring
...
def device_matches_person(pbox, dbox):
    """
    Conservative: device haqiqatan ham shu person'ga tegishli bo'lsa True.
    Shart: IoU >= 0.05 YOKI (device markazi person ichida va pastki 70% da)
    """
...
def main():
    ...
    # device nomlarini tekshirish
    dev_names = {int(k): str(v).lower() for k,v in device_model.names.items()}

    idx = 0; saved = 0; skipped_due_to_device = 0
    while True:
        ret, frame = cap.read()
        if not ret: break
        idx += 1
        ...
        # 2) DEVICE (pastroq conf bilan) + class-wise NMS
        ...
        # 3) Har bir person uchun “device bor-yo‘qligini” aniq tekshiramiz
        for i,pbox in enumerate(persons):
            # tekshiruvni “kengaytirilgan” person box bilan qilamiz
            ...
            for dbox, dconf, dname in devs:
                if device_matches_person(pbox_chk, dbox):
                    ...
                    break
            if has_device:
                skipped_due_to_device += 1
                continue  # bu person “no_device” EMAS

            # 4) “no_device” crop saqlaymiz
            x1,y1,x2,y2 = pbox
            crop = frame[y1:y2, x1:x2]
            if crop.size == 0:
                continue
            fname = f"nd_{idx:06d}_{i}.jpg"
            cv2.imwrite(str(Path(OUT, fname)), crop)
            saved += 1

    cap.release()
    print(f"[DONE] Saved no_device: {saved} | Skipped (device present): {skipped_due_to_device}")
